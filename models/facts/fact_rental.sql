...
rental_base_2 AS (
    SELECT
        ...
        (
            CASE
                WHEN dim_staff.staff_id IS NOT NULL THEN dim_staff.staff_id
                ELSE -1
            END
        ) AS staff_id_rental_check,
        (
            CASE
                WHEN dim_customer.customer_id IS NOT NULL THEN dim_customer.customer_id
                ELSE -1
            END
        ) AS customer_id_check,
        (
            CASE
                WHEN dim_film.film_id IS NOT NULL THEN dim_film.film_id
                ELSE -1
            END
        ) AS film_id_check,
        (
            CASE
                WHEN dim_store.store_id IS NOT NULL THEN dim_store.store_id
                ELSE -1
            END
        ) AS store_id_check
    FROM
...
SELECT
    {{ dbt_utils.generate_surrogate_key (['rental_id', 'rental_date',]) }} AS surrogate_id,
    rental_id,
    rental_date,
    date_key,
    inventory_id,
    ...
    CASE
        WHEN new_return_date IS NOT NULL THEN diff / 3600
        ELSE NULL
    END AS rental_hours,
    is_return,
    last_update,
